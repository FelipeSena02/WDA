...
if(!empty($_GET['search']))
    {
        $data = $_GET['search'];
        $sql = "SELECT * FROM aluguel WHERE id LIKE '%$data%' or livro LIKE '%$data%' or usuario LIKE '%$data%' 
        or data_aluguel LIKE '%$data%' or previsao LIKE '%$data%' or devolucao LIKE '%$data%' ORDER BY id DESC";
    }
    else
    {
        $sql = "SELECT * FROM aluguel ORDER BY id DESC";
...
<html>
    ...
    <body class="fundo">
        ...
        <br>
        <button><a href="form_aluguel.php"><h3> Cadastrar Aluguel</h3></a></button>
                ...
                <thead>
                    ...
                    <tbody>
                        <?php
                            ...
                            {
                                $data_aluguel=date("d/m/Y",strtotime($user_data['data_aluguel']));
                                $previsao=date("d/m/Y",strtotime($user_data['previsao']));
                                ...
                                echo "<td>".$previsao."</td>";
                                if($user_data['devolucao']=="") {
                                    echo "<td>"
                                    ."<a class='btn btn-sm btn-primary' href='edit_aluguel.php?id=$user_data[id]'>
                                    ...
                                    "</td>";
                                    echo "<td>" .$user_data['devolucao']."<br>No prazo" . "</td>";
                                    }else{
                                        echo "<td>"
                                    ."<a class='btn btn-sm btn-danger' href='delete_aluguel.php?id=$user_data[id]'>
                                    <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-trash-fill' viewBox='0 0 16 16'>
                                    ...
                                    "</td>";
                                    echo "<td>" .$user_data['devolucao']."<br>Atrasado" . "</td>";
                                    }
                                }                           
                                echo "</tr>";
                            }
